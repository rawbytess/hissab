...
// The stable `kind` field on each token must mirror the camelCase of its class
// name (the historical syntax-highlighting key) so highlighting survives
// production minification, where `constructor.name` is mangled.
const expectedKind = (className: string) =>
  className === "VariableNameToken"
    ? "VariableNameToken"
  ...
  readdirSync(validTestDir).forEach((file) => {
    ...
    exps.forEach((exp, i) => {
      ...
      test(`Valid Tokens:${filename}:${i + 1}`, () => {
        ...
        expect(tokens[0].kind).toBe(expectedKind(filename));
      });
    });
  });
});
...
describe("Invalid Tokens", () => {
  ...
  exps.forEach((exp, i) => {
    ...
    test(`Invalid Tokens:${i + 1}`, () => {
      ...
      expect(tokens[0].kind).toBe("undefinedToken");
    });
  });
});
...
describe("Valid Token Types", () => {
  ...
  for (const key in Synonyms) {
    test(`Synonyms token ${key}`, () => {
      ...
      // Compound-shaped synonyms (`mph → mile per hour`, `psi → pound per
      // square inch`) intentionally expand into multiple tokens for the
      // parser's compound-unit absorption to consume.
      const isCompoundSynonym = /\bper\b/.test(Synonyms[key].value);
      if (!isCompoundSynonym) expect(tokens.length).toBeLessThanOrEqual(2);
