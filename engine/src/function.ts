...
async function minimum(...allparams: any[]): Promise<NumberToken> {
  ...
  allparams[2](true);
  if (params) {
    const sameParams = await getSameParams(params, params[0].unit?.value, true);
...
function mode(...numbers: number[]) {
  ...
  for (const num of numbers) occurrences[num] = (occurrences[num] || 0) + 1;

  let modeList: number[] = [];
  let maxFrequency = 0;

  for (const num in occurrences) {
    if (occurrences[num] > maxFrequency) {
      modeList = [parseFloat(num)];
      maxFrequency = occurrences[num];
    } else if (occurrences[num] === maxFrequency)
      modeList.push(parseFloat(num));
  }
